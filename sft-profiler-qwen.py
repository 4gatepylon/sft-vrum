    ...
    def cosine_decay(self, itr):
        if itr < self.warmup_itr:
            ...
            lr = self.min_lr + (self.max_lr - self.min_lr) * 0.5 * (
                math.cos(math.pi * decay_frac) + 1.0
            )
        return lr
...
def main():
    ...
    original_grad_checkpointing_frequency = opt_config.grad_checkpointing_frequency
    qwen_config = QwenConfig()
    n_layers = qwen_config.nbr_layer
    for grad_checkpointing_frequency in range(1, n_layers + 1):
        try:
            opt_config.grad_checkpointing_frequency = grad_checkpointing_frequency
            # set up profiling
            logging.basicConfig(
            ...
            with torch.profiler.profile(
                ...
                with record_function("## initialization ##"):
                    ...
                    train_log = os.path.join(log_dir, f"train_log_ck_freq_{grad_checkpointing_frequency}.txt")
                ...
                for itr in range(opt_config.max_itr):
                    ...
                    for min_itr in range(opt_config.grad_accum_steps):
                        ...
                        with record_function("## forward ##"):
                            ...
                            if torch.cuda.is_available():
                                with torch.autocast(
                                    device_type=opt_config.device_type,
                    ...
                    if itr % print_int == 0:
                        ...
                        print(
                            f"iter {itr} | train loss: {avg_loss.item():.4f} | walltime: {min_dt:.2f} sec | ktoks/sec: {TPS/1e3:.2f}"
                        )
                        with open(train_log, "a") as f:
                            f.write(
                                f"{itr:5d} |    {avg_loss.item():.4f} |           {min_dt:.2f} |      {TPS/1e3:.2f}\n"
                            )
            ...
            print(f"Error: {e}")
            print(f"grad_checkpointing_frequency: {grad_checkpointing_frequency}")
            # raise e
            pass
    opt_config.grad_checkpointing_frequency = original_grad_checkpointing_frequency


if __name__ == "__main__":
    main() # Just try all the frequencies? lmao
